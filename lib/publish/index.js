...
const { isExists, exec, upgrade } = require('../../utils/helper')
...
module.exports = async options => {
    ...
    try {
        ...
        let onlineVersion = await getOnlineVersion({ name, registryUri }, spinner)
        if (!version) {
            version = upgrade(semver.valid(semver.coerce(onlineVersion)), 1)
        }
        await updatePackage({ version, pkg }, spinner)
        await publishPackage({ version, registryUri }, spinner)
        spinner.stop()
    } catch (err) {
